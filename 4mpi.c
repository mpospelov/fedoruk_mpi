...
void makeGNUplotMPI(double** , long);
...
int main( int argc, char **argv ){
  ...
  inicialBorderConditionsTwo(borderConditionTwo, 0.0); // heat temp in x[i][last] = temp
  ...
  if (yPoints % total != 0) {
    printf("\n++++++++++++Error yPoints!!!! yP = %ld, total = %d\n", yPoints, total);
    //yPoints += total - yPoints % total;
    //printf("\nChange number to = %ld \n", (yPoints+1)*xPoints);
    //exit(1);
  }
  // > Mpi Init
  ...
  if (myrank == 0) {
    ...
    ftime(&tmb); // точка начала расчетов
    time0 = (long long)tmb.time * 1000 + tmb.millitm;
  }
  ...
  for (i = 0; i < (calcTime / dt); i++) {
    ...
    if (flag == 0) {
      ...
      if (myrank == 0) {
        ...
        if (debug == 1) solve(tempMas2, tempMasPrev, borderConditionOne, borderConditionTwo, i);
      }
      ...
      if (myrank == 0) {
        if (debug == 1) printMasI(resizeTempMasPrev, xPoints, yPointsMPI);

        if (debug == 1) printf("\nMake array for GNU\n");
        makeArrayForSendToGNU(resizeTempMasPrev, tempMasPrev, total);

        if (debug == 1) compareTwoArray(tempMasPrev, tempMas2, xPoints, yPoints);

        if (gnu == 1) {
          makeGNUplotMPI(tempMasPrev, i);
        }
      }
      ...
      if (myrank == 0) {
        ...
        if (debug == 1) solve(tempMasPrev2, tempMas, borderConditionOne, borderConditionTwo, i);
      }
      ...
      if (myrank == 0) {
        if (debug == 1) printMasI(resizeTempMas, xPoints, yPointsMPI);

        if (debug == 1) printf("\nMake array for GNU\n");
        makeArrayForSendToGNU(resizeTempMas, tempMas, total);

        if (debug == 1) compareTwoArray(tempMas, tempMasPrev2, xPoints, yPoints);

        if (gnu == 1) {
          makeGNUplotMPI(tempMas, i);
        }
      }
  ...
  if (myrank == 0) {
    ftime(&tmb); // точка конца расчетов
    time1 = (long long)tmb.time * 1000 + tmb.millitm;
    printf("\nEND! time = %lld \n", time1 - time0);
  }
...
void makeGNUplotMPI(double** temp, long calcIterat) {
  if (gnu == 1) {
  ...
  for (i = 0; i < yPoints; i++) {
    for (j = 0; j < xPoints; j++){
      ...
      if (borderTwo[i][j] != defCondOne) {
        ...
        if (i == 0) {
          temp[i][j] = borderTwo[i][j] * dy + tempPrev[i+1][j];
        }
        if (i == yPoints-1) {
          temp[i][j] = borderTwo[i][j] * dy + tempPrev[i-1][j];
        }
